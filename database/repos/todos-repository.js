module.exports = class TodosRepository {
    ...
    async getStatus(status) {
        return this.db.manyOrNone('SELECT * FROM todos WHERE status = $1', [status])
    }
    async add(params) {
        return this.db.one(this.pgp.helpers.insert(params, this.insertCS) + ' RETURNING *;')
    }
    async updateTodo(params) {
        ...
        //     params
        // })
        return this.db.one(this.pgp.helpers.update(params, this.updateCS) + `WHERE id = ${params.id} RETURNING *;`)
    }
    async remove(id) {
